...
export class Runner implements IRunner{
    ...
    lastName2: string = "";
    status: "" | "DNS" | "DNF" | "DSQ" = "";
    constructor(opts?: object | IRunner | null){
        if (typeof opts == IRunner)
        if(opts.Firstname && opts)
    }
}
export class RunnerResult extends Runner implements IRunnerResult{
...
export class Spotters implements ISpotters{
    ...
    getSpotter(name: string): IRunner | null {
        ...
        for(const runner of this.runners){
            if ((runner.lastName = name || runner.lastName2) === name)
            return runner;
        }
        return null;
    }
